...
- Unified write modes via a shared `SheetWriteMode` (`FAIL_IF_EXISTS` / `REPLACE` / `APPEND`) describing the disposition toward existing content. `writeSheet` (`SheetWriteOptions`, default `FAIL_IF_EXISTS`) governs the named sheet; the row writers `Sheet.putRows` (`WriteOptions`, default `APPEND`, nullable `startRowIndex` that resolves to the mode's natural point) and `Sheet.setRow` (`RowWriteOptions`, default `REPLACE`) govern the rows they write — `APPEND` inserts and shifts existing content down rather than overwriting, `FAIL_IF_EXISTS` writes only into empty rows. `writeTable` / `Table.putRows` take `TableWriteOptions` (`tableWriteMode`: `REPLACE` default / `APPEND`, plus an APPEND-only `insertAt` data-row index for inserting inside the table); a table is self-describing, so there are no positional or header fields, and a resize that would shift another table fails with a `TableOverlapError`. Inserting *sheet* rows (`Sheet.putRows` / `setRow` with `APPEND`) into a table's region is refused with the same error — a table is modified through the Table API, not by shifting its rows from the sheet.
