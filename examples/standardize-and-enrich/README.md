...
processing stamp), and writes the standardized result as a new Excel file. The input is read as a byte
...
4. Writes the standardized, enriched rows to `resources/standardized_sales.xlsx` (`writeSheet`).

The run prints the source and output row counts; compare `resources/source_sales.xlsx` with
`resources/standardized_sales.xlsx` to see the standardization and enrichment.
